%-------------------------------------------------------------------------
% script file: sarsa_windy_gridworld_stochastic_wind.m
% rbd
% ray.duran@und.edu
% EE547
% Last update: 8/2/20
...
for n = 1: episodes
    ...
    while windsurfing
        ...
        stoch_wind_factor = randi(3,1);
        
        switch j_update
            case 1
             if( stoch_wind_factor == 1) % mean 
              i_update = i_update;
             elseif( stoch_wind_factor == 2) % -1 strength 1 greater mean
              i_update = i_update-1;
             else % == 3 ; +1 strength 1 less
              i_update = i_update+1;
             end
            case 2
             if( stoch_wind_factor == 1) 
                i_update = i_update;
             elseif( stoch_wind_factor == 2) 
                i_update = i_update-1;
             else
                i_update = i_update+1;
             end
